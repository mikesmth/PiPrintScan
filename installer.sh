...
#root check
if      [ $USER != root ]; 
then    echo  "Error: must be root"
        exit 0
fi

check_dep () {
if      [ "$(which pip)" ];
then    echo "pip found, trying to update it"
        pip install -U pip

else    wget -q https://bootstrap.pypa.io/get-pip.py
  if      [ "$(which python)" ]; 
  then    python get-pip.py
  else    apt-get install -q python2.7
  fi
fi
...
if      [ "$(which npm)" ]; then
        ...
        npm upgrade npm -g
else    echo "Downloading $1 [$CURRENT_FILE/$AMOUNT_FILES]"
...
else    pip install cloudprint
fi

if      [ "$(which mintr)" ]; then
		echo "mintr found, trying to update it"
		npm upgrade mintr -g
else    npm install mintr -g
fi

if      [ "$(which git)" ]; 
then	echo "git found"
if      [ -d "/usr/bin/airprint" ]; 
then	cd /usr/bin/airprint
		git pull
else    git clone https://github.com/tjfontaine/airprint-generate /usr/bin/airprint
fi
else    apt-get -q -y git
...
if      [ -f /etc/cups/cups-pdf.conf ]; 
then    sed -i "s|Out ${HOME}/PDF|Out /storage/pdf|" /etc/cups/cups-pdf.conf
	    sed -i "s|#PostProcessing|PostProcessing /usr/local/bin/cups-pdf-renamer|" /etc/cups/cups-pdf.conf
else    echo "cups-pdf.conf does not exists"
fi

if      [ -f /etc/cups/cupsd.conf ]; 
then	echo  -e "nServerName $name" >> /etc/cups/cupsd.conf
...
else    echo "cupsd.conf does not exists"
fi  

if [ -f /etc/cups/printers.conf ]; 
then	sed -i "s|Shared No|Shared Yes|g" /etc/cups/printers.conf
else	echo "printers.conf does not exists"
fi  
...
else    echo "smb.conf does not exists"
fi
